...
- b397311: 중첩 테마 스코프: `tokens.css`와 `createTheme` 생성 CSS가 `[data-dds-theme="light"]` 블록을 추가로 내보낸다. 다크 블록이 재정의하는 semantic 색 47개 전부를 `:root`와 같은 라이트 값으로 담아, `<html data-dds-theme="dark">` 아래에서 `data-dds-theme="light"`를 붙인 영역이 더 이상 다크 값을 상속하지 않는다. 다크 안의 라이트, 라이트 안의 다크, 여러 겹 중첩 모두 가장 가까운 조상을 따른다. `:root` 기본값은 그대로다.
...
- 8440042: createTheme — 브랜드 hex 하나로 WCAG 검사를 통과한 테마 생성
