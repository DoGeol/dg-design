 ...
 */
export const LIGHTNESS = [0.97, 0.929, 0.869, 0.76, 0.64, 0.54, 0.45, 0.36, 0.27, 0.18];
...
/**
 * 스텝별 "sRGB 상한 대비 비율". 위 brand 램프(hue 195, 수작업)의 실측값이고,
 * 다른 hue의 램프는 이 프로파일 × 해당 (L, hue) 상한으로 파생한다 — 균일 비율을 쓰면
 * 밝은 영역 상한이 큰 hue(green 등)에서 네온색이 튀어 램프 패밀리가 깨진다.
 *
 * brand·gray는 이 프로파일이 아니라 수작업 배열을 그대로 쓴다(기본 팔레트 불변).
 */
export const CHROMA_RATIO_PROFILE = [0.69, 0.54, 0.64, 0.77, 0.84, 0.89, 0.88, 0.89, 0.89, 0.86];
 ...
 * 해당 (L, hue) 상한 × `CHROMA_RATIO_PROFILE`을 손으로 계산해 굳힌 값 —
 * 넘기면 generate가 gamut 검사에서 실패한다.
 */
