...
0.9.0 검토(렌즈 6개 감사)에서 나온 31건 중 **27건이 처리됐고 3건이 남았다.** 남은 셋은 미루다 남은 게 아니라 **지금 하지 않는 것이 맞다고 판단된 것**들이라, 각각 "무엇이 정해져야 착수할 수 있는지"를 같이 적는다.
...
### A5 — Tabs · Accordion · Avatar · Skeleton

**문제 아님**: 흔한 컴포넌트지만 지금 막고 있는 것은 없다.

**착수 조건**: **소비 앱이 요구할 때.** 이번 감사가 보여준 게 정확히 이것이다 — 소비자 없이 쌓은 17종에서 문제가 전부 "앱에 넣어야 보이는" 층(중첩 오버레이, z-index, 트리셰이킹, Field 연동)에 몰려 있었다. 컴포넌트를 더 늘리는 것보다 DDS를 실제로 쓰는 앱을 하나 만드는 쪽이 다음 판단들의 근거를 만든다.

### 릴리스 운영 — `publish`는 사람이 한다

npm 재인증(2FA) 때문에 `changeset publish`는 사용자가 터미널에서 직접 실행한다. 스코프 패키지는 권한이 없을 때 403이 아니라 **404(PUT)**로 나오므로, 그건 "패키지 없음"이 아니라 재인증 신호다(`npm whoami`가 401이면 확정).
